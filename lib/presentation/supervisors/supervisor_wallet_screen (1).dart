...
final advanceSupervisorsProvider =
...
// SupervisorPaymentTabBody below — kept around unchanged for any existing
// direct link to '/advance-payment', and so its behavior is identical to
// before. The actual search+list content lives in SupervisorPaymentTabBody
// so PaymentScreen's "Supervisors" tab can reuse the exact same widget
// instead of a second, potentially-diverging copy.

class AdvancePaymentScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SupervisorPaymentTabBody(supervisorId: supervisorId),
    );
  }
}

/// The search+list body for paying/advancing supervisors — no Scaffold or
/// AppBar of its own, so it can be embedded either standalone (via
/// AdvancePaymentScreen above) or as one tab of PaymentScreen.
class SupervisorPaymentTabBody extends ConsumerStatefulWidget {
  final String? supervisorId;
  const SupervisorPaymentTabBody({super.key, this.supervisorId});

  @override
  ConsumerState<SupervisorPaymentTabBody> createState() =>
      _SupervisorPaymentTabBodyState();
}

class _SupervisorPaymentTabBodyState
    extends ConsumerState<SupervisorPaymentTabBody> {
  ...
  @override
  Widget build(BuildContext context) {
    final supervisorsAsync = ref.watch(advanceSupervisorsProvider);
          ...
          child: supervisorsAsync.when(
            ...
            data: (supervisors) {
              ...
              return RefreshIndicator(
                onRefresh: () async =>
                    ref.invalidate(advanceSupervisorsProvider),
