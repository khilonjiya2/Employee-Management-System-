...
class PayrollDetailScreen extends ConsumerWidget {
  ...
  Widget _buildAttendanceSummary(BuildContext context, PayrollModel p) {
    return _Card(
      ...
      children: [
        _PayRow(
            label: 'Present Days',
            value: '${p.presentDays} days',
            bold: false),
        _PayRow(
            label: 'Half Days',
            value: '${p.halfDays} days',
            bold: false),
        _PayRow(
            label: 'Absent Days',
            value: '${p.absentDays} days',
            bold: false),
        _PayRow(
            label: 'Leave Days',
            value: '${p.leaveDays} days',
            bold: false),
        const Divider(),
        _PayRow(
            label: 'Effective Days',
            value: '${p.effectiveDays.toStringAsFixed(1)} days',
            bold: true),
        _PayRow(
            label: 'Daily Wage Rate',
            value: CurrencyUtils.format(p.dailyWageRate),
            bold: false),
      ],
    );
  }
      ...
      children: [
        _PayRow(
            ...
            value: CurrencyUtils.format(
                p.effectiveDays * p.dailyWageRate)),
        if (p.overtimeAmount > 0)
          _PayRow(
              label: 'Overtime',
              value: '+ ${CurrencyUtils.format(p.overtimeAmount)}',
              valueColor: AppColors.success600),
        if (p.bonus > 0)
          _PayRow(
              label: 'Bonus',
              value: '+ ${CurrencyUtils.format(p.bonus)}',
              valueColor: AppColors.success600),
        if (p.advanceDeduction > 0)
          _PayRow(
              label: 'Advance Deduction',
              value: '- ${CurrencyUtils.format(p.advanceDeduction)}',
              valueColor: AppColors.error600),
        if (p.penaltyDeduction > 0)
          _PayRow(
              label: 'Penalty',
              value: '- ${CurrencyUtils.format(p.penaltyDeduction)}',
              valueColor: AppColors.error600),
        const Divider(),
        _PayRow(
            label: 'Gross Wage',
            value: CurrencyUtils.format(p.grossWage)),
        _PayRow(
            label: 'Net Wage',
  ...
  Widget _buildPaymentSection(
      BuildContext context, WidgetRef ref, PayrollModel p) {
    if (p.isPaid) {
      return Container(
        ...
        if (paymentEnabled)
          ElevatedButton.icon(
            icon: const Icon(Icons.account_balance_wallet_rounded,
                size: 18),
            label:
                Text('Pay ${CurrencyUtils.format(p.netWage)} via UPI'),
            style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.success500),
            onPressed: () =>
                w.UpiPaymentHelper.payPayroll(context, ref, p),
          ),
