...
/// A single employee's payments for ONE specific month/year — this is
/// the month-scoped view: total paid that month, and the list of
/// transactions within it. Keyed by a record so Riverpod treats each
/// (employee, month, year) combination as its own cached entry.
typedef EmployeeMonthKey = ({String employeeId, int month, int year});

class EmployeeMonthPayments {
  final double total;
  final List<EmployeePaymentModel> transactions;
  const EmployeeMonthPayments({required this.total, required this.transactions});
}

final employeeMonthPaymentsProvider = FutureProvider.autoDispose
    .family<EmployeeMonthPayments, EmployeeMonthKey>((ref, key) async {
  final all = await ref.read(employeePaymentRepositoryProvider).getPayments(key.employeeId);
  final filtered = all
      .where((p) => p.paymentMonth == key.month && p.paymentYear == key.year)
      .toList();
  final total = filtered.fold<double>(0, (sum, p) => sum + p.amount);
  return EmployeeMonthPayments(total: total, transactions: filtered);
});
...
class EmployeePaymentRepository {
  ...
  Future<void> editPayment({
    ...
    await _client.rpc('edit_employee_payment', params: {
      ...
      if (newNote != null) 'p_new_note': newNote,
    });
  }
}
